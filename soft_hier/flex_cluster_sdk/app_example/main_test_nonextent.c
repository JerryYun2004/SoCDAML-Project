...
int main(void)
{
    ...
    if (cid == 0u && core == 0u) {
        printf("[Baseline/Non-Extended] 4x4 GEMM (each cluster loads from HBM; no inter-cluster broadcast)\n");
    ...
    /* ===========================================================
     * Baseline flow (NO BROADCAST):
     *   For each cluster C[ry,rx], the DM core:
     *     1) Loads A-strip r=ry from HBM with 1D contiguous DMA
    ...
    for (uint32_t ry = 0; ry < 4u; ++ry) {
        for (uint32_t rx = 0; rx < 4u; ++rx) {
            ...
            if (IS_DM && P.y == ry && P.x == rx) {
                ...
                /* --- 2) Load B-strip c=rx (2D gather of column-block) --- */
                ...
                printf("[Load][B] C[%u,%u](DM) loads B-strip c=%u from HBM base=0x%08x bytes/row=%u repeat=%u\n",
                       (unsigned)ry, (unsigned)rx, (unsigned)rx, (unsigned)offB_base,
    ...
    if (cid == 0u && core == 0u) {
        printf("[Done/Baseline] All 16 tiles of C written to HBM at base 0x%08x\n",
